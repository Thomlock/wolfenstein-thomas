...
func _process(delta):
	time_since_hast_shot += delta
	can_shoot = time_since_hast_shot >= (1.0/ fire_rate)
...
func update_weapon_icon():
	if $WEAPON.texture != weapon_texture[Global.current_weapon]:
		$WEAPON.texture = weapon_texture[Global.current_weapon]
